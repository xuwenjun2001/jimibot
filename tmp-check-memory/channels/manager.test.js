...
const manager = new ChannelManager(config, bus, {
    telegram: (channelConfig, channelBus) => new MockChannel("telegram", channelConfig, channelBus),
});
...
assert.equal(channel.sentMessages[0]?.content, "hello outbound");
bus.publishOutbound(createOutboundMessage({
    channel: "telegram",
    chatId: "chat-2",
...
const unknownRouted = await manager.routeOutboundMessage(createOutboundMessage({
    channel: "unknown",
    chatId: "chat-3",
    content: "missing channel",
}));
assert.equal(unknownRouted, false);
await channel.simulateInboundMessage({
    senderId: "u1",
    chatId: "chat-1",
