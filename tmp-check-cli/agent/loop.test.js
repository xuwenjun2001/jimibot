...
{
    const { workspacePath, sessionManager } = await createLoopHarness("agent-loop-skills-context");
    ...
    await writeFile(path.join(workspacePath, "skills", "memory", "SKILL.md"), [
        "---",
    ...
    await writeFile(path.join(workspacePath, "skills", "docker", "SKILL.md"), [
        "---",
        "description: Use Docker when containerization is needed.",
        "---",
        "Use this skill to build and run containers.",
    ].join("\n"), "utf-8");
    await writeFile(path.join(builtinSkillsPath, "cron", "SKILL.md"), [
        "---",
...
{
    const { workspacePath, bus, sessionManager } = await createLoopHarness("agent-loop-subagent");
    ...
    assert.match(String(mainProvider.requests[0]?.tools?.map((tool) => tool.function.name).join(",")), /spawn/);
    assert.match(String(mainProvider.requests[2]?.messages.at(-1)?.content ?? ""), /Background task 'inspect workspace' completed successfully/);
    assert.equal(subagentProvider.requests.length, 2);
    const subagentToolNames = subagentProvider.requests[0]?.tools?.map((tool) => tool.function.name).sort() ??
        [];
