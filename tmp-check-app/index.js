...
const DEFAULT_CONFIG_FILENAME = "nanobot.config.json";
export async function createNanobotApp(options = {}) {
...
export async function runNanobot(options = {}) {
    const app = await createNanobotApp(options);
    await app.start();
    return app;
}
...
async function main() {
    ...
    const app = await runNanobot(runOptions);
    installSignalHandlers(app);
    console.log("Nanobot TS starting...");
...
if (isMainModule()) {
    void main().catch((error) => {
        ...
        console.error(`Failed to start Nanobot TS: ${message}`);
        process.exitCode = 1;
    });
}
