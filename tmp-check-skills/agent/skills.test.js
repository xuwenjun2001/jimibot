...
await writeFile(path.join(builtinSkillsPath, "memory", "SKILL.md"), [
    "---",
...
assert.deepEqual(allSkills.map((skill) => ({
    name: skill.name,
    source: skill.source,
...
assert.deepEqual(availableSkills.map((skill) => skill.name), ["cron", "memory"]);
...
assert.match(String(memorySkill?.content ?? ""), /stable user preferences and project facts/i);
const alwaysSkills = await loader.getAlwaysLoadSkills();
assert.deepEqual(alwaysSkills.map((skill) => skill.name), ["cron", "memory"]);
