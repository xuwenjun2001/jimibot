...
assert.equal(await store.loadMemory(), "# Long-term Memory\n- User likes TypeScript");
await store.appendHistory("Discussed the refactoring roadmap.");
const initialHistory = await store.loadHistory();
assert.match(initialHistory, /^\[\d{4}-\d{2}-\d{2} \d{2}:\d{2}\] Discussed the refactoring roadmap\.\r?\n$/);
            ...
            toolCalls: [
                {
                    ...
                    name: "save_memory",
                    arguments: {
                        history_entry: "[2026-03-14 09:05] User is learning jimibot and prefers TypeScript examples.",
...
assert.match(finalHistory, /\[2026-03-14 09:05\] User is learning jimibot and prefers TypeScript examples\./);
